...
organizerRouter.post('/update/:id',
  (req: Request, res: Response) => {

    return apiResponse.error(res, 'Organizer not found', []);

    let checkIdQuery = "SELECT * FROM organizers WHERE id = ?";
    let checkIdParams = [req.params.id];
    ...
    let {name, description, about} = req.body;


    let query = "UPDATE organizers SET name = ?, description = ?, about = ?, updated_at = ? WHERE id = ?";
    let params = [name, description, about, HelperFunction.getDateTime(0), req.params.id];
   
    
    return apiResponse.success(res, 'Update is Successful', []);    
